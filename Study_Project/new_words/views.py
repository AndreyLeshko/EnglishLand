...
@login_required
def user_words_train(request):
    cur_train = Train.objects.filter(user=request.user).filter(status='on study').select_related('word').order_by('?').first()
    # print('\n\n\n')
    # print(word)
    # print('\n\n\n')
    return render(request, 'new_words/user_words_train.html', context={'cur_train': cur_train})


@login_required
def user_words_train_result(request):
    ...
    return render(request, 'new_words/user_words_train_result.html', context=context)
